...
def _aggregate_project_level(rows_df: pd.DataFrame | list[dict], value_col: str) -> pd.DataFrame:
    """Aggregate fold-level rows to project-level by median over folds (per strategy).

    Returns a wide DataFrame indexed by project with one column per strategy containing
    the median of finite values in `value_col` across folds for that project/strategy.
...
def main():
    ...
    if args.emit_waste_csv and rows_target_eligible and args.target_found is not None:
        ...
        # best per (project, fold)
        best = (df_e.loc[df_e.groupby(["project","fold"])['required_builds'].idxmin(),
                        ["project","fold","required_builds"]]
                    .rename(columns={"required_builds":"best_required_builds"}))
        df_w = df_e.merge(best, on=["project","fold"], how="left")
