...
def coerce_datetime_utc(series):
    """Parse datetimes and normalize to UTC tz-aware pandas Series.
    If naive, localize as UTC (best-effort). Returns pandas Series[datetime64[ns, UTC]].
    """
...
def compute_a_values(df: pd.DataFrame, r1: float, r2: float) -> pd.DataFrame:
    ...
    # 論文に忠実な写像：S_new = LA+LD、S_reuse = LT - S_new（LT: 変更前の行数）
    if "S_new" in df.columns:
        S_new_i = pd.to_numeric(df["S_new"], errors="coerce").fillna(0).astype(float)
    else:
        if ("lines_added" in df.columns) and ("lines_deleted" in df.columns):
            S_new_i = (pd.to_numeric(df["lines_added"], errors="coerce").fillna(0).astype(float)
                       + pd.to_numeric(df["lines_deleted"], errors="coerce").fillna(0).astype(float))
        else:
            S_new_i = safe_change_size(df)

    if "S_reuse" in df.columns:
        S_reuse_i = pd.to_numeric(df["S_reuse"], errors="coerce").fillna(0).astype(float)
    elif "total_prev_loc" in df.columns:
        LT = pd.to_numeric(df["total_prev_loc"], errors="coerce").fillna(0).astype(float)
        # 改善: 再利用は「変更前の既存行から削除された行を引いたもの」とみなす
        # S_reuse = max(0, total_prev_loc - lines_deleted)
        if "lines_deleted" in df.columns:
            LD = pd.to_numeric(df["lines_deleted"], errors="coerce").fillna(0).astype(float)
            S_reuse_i = (LT - LD).clip(lower=0)
        else:
            # フォールバック: 利用可能な情報が無い場合は従来近似を維持
            S_reuse_i = (LT - S_new_i).clip(lower=0)
    else:
        S_reuse_i = pd.Series(0.0, index=df.index)
...
def estimate_b0_monden(df_train: pd.DataFrame, builds_per_day: int,
    ...
    # 日付（date）と日時（UTC）の両方を用意
    vdf["commit_date_date"]   = coerce_date_only(vdf.get("commit_date"))
    vdf["reported_date_date"] = coerce_date_only(vdf.get("reported_date"))
    # 可能なら datetime 列を優先し、無い場合は date 列で代替
    commit_dt_src = vdf.get("commit_datetime") if "commit_datetime" in vdf.columns else vdf.get("commit_date")
    reported_dt_src = vdf.get("reported_datetime") if "reported_datetime" in vdf.columns else vdf.get("reported_date")
    vdf["commit_dt_utc"]   = coerce_datetime_utc(commit_dt_src)
    vdf["reported_dt_utc"] = coerce_datetime_utc(reported_dt_src)
    # 検出基準（reported_date）で学習期間に含まれる脆弱事象を抽出
    vtrain = vdf[(vdf["reported_date_date"] >= train_start) & (vdf["reported_date_date"] <= train_end)].copy()
    if vtrain.empty:
        return 0.01

    # 学習期間の df_train 全体を用意（サイズ・a_i は集合全体の総和で評価する）
    ...
    # 可能なら UTC の datetime 差分（日単位、切り上げ）を使用。無ければ date 差分（日単位）にフォールバック
    if "reported_dt_utc" in merged.columns and "commit_dt_utc" in merged.columns:
        try:
            delta_days = (merged["reported_dt_utc"] - merged["commit_dt_utc"]).dt.total_seconds() / 86400.0
            days = np.ceil(pd.to_numeric(delta_days, errors="coerce")).fillna(1).astype(int)
            days = days.clip(lower=1)
        except Exception:
            days = (pd.to_datetime(merged["reported_date_date"]) - pd.to_datetime(merged["commit_date_date"]))\
                        .dt.days.fillna(1).astype(int)
            days = np.maximum(1, days)
    else:
        days = (pd.to_datetime(merged["reported_date_date"]) - pd.to_datetime(merged["commit_date_date"]))\
                    .dt.days.fillna(1).astype(int)
        days = np.maximum(1, days)
    ...
    # 論文の式(5)の整合に合わせ、S_tot と a_tot は学習集合（df_train）全体で集計する
    # 学習期間中に検出された脆弱事象数（reported基準）
    H_tot = len(vtrain)
    # a_tot: df_train 全行の a_i 総和（欠損は0）
    a_tot = float(pd.to_numeric(df_train.get("a_i"), errors="coerce").fillna(0.0).sum())
    # S_tot: df_train 全行の change_size 総和（下限処理なし）
    S_tot = float(pd.to_numeric(safe_change_size(df_train), errors="coerce").fillna(0.0).sum())
    # t_tot: 学習期間の基準努力（calendar相当: 期間日数×builds_per_day）
    try:
        span_days = (pd.to_datetime(train_end) - pd.to_datetime(train_start)).days + 1
    except Exception:
        span_days = 0
    t_tot = int(max(1, builds_per_day)) * int(max(1, span_days))
...
def expected_found_curve(df_test: pd.DataFrame, budgets: np.ndarray, b0: float, risk_col: str) -> dict:
    ...
    # サイズは下限1でクリップして安定化（固定回数側と整合）
    S_raw = pd.to_numeric(safe_change_size(df_test), errors="coerce").fillna(0.0).values.astype(float)
    S_i = np.maximum(S_raw, 1.0)
...
# ======== 図描画（固定回数Fuzz: 単系列可視化） ========
def plot_ecdf_single(values, xlabel, title, out_png, percent_y=False):
    arr = np.asarray(values, float)
    arr = arr[np.isfinite(arr)]
    fig = plt.figure(figsize=(8.5,5.5)); ax = fig.add_subplot(1,1,1)
    if arr.size > 0:
        xs, ys = ecdf_from_values(arr)
        ax.step(xs, ys* (100.0 if percent_y else 1.0), where='post')
    ax.set_title(title)
    ax.set_xlabel(xlabel)
    ax.set_ylabel('Cumulative % of Projects' if percent_y else 'Cumulative fraction of projects')
    if percent_y:
        ax.yaxis.set_major_formatter(mticker.PercentFormatter())
    ax.grid(True, linestyle='--', alpha=0.6)
    fig.savefig(out_png, dpi=300, bbox_inches='tight'); plt.close(fig)

def plot_violin_box_single(values, ylabel, title, out_png, percent_y=False):
    arr = np.asarray(values, float)
    arr = arr[np.isfinite(arr)]
    stats_csv = os.path.join(os.path.dirname(out_png), os.path.basename(out_png).replace('.png','_stats.csv'))
    save_distribution_stats({"FixedRuns": arr.tolist()}, stats_csv)
    fig, ax = plt.subplots(figsize=(8.5,5.5))
    if arr.size > 0:
        parts = ax.violinplot([arr], showmeans=False, showmedians=False, showextrema=False)
        for pc in parts['bodies']:
            pc.set_alpha(0.25); pc.set_edgecolor('black'); pc.set_linewidth(0.5)
        bp = ax.boxplot([arr], labels=['FixedRuns'], showfliers=True, widths=0.28, patch_artist=True, showmeans=True,
                        meanprops=dict(marker='D', markersize=6, markeredgecolor='black', markerfacecolor='green', zorder=5))
        for patch in bp['boxes']:
            patch.set_alpha(0.6); patch.set_facecolor('lightgray'); patch.set_edgecolor('black')
        for line in bp['whiskers'] + bp['caps'] + bp['medians']:
            line.set_linewidth(1.2)
    else:
...
def plot_scatter_cost_vs_found(x_vals, y_vals, x_label, y_label, title, out_png, percent_x=False):
    x = np.asarray(x_vals, float); y = np.asarray(y_vals, float)
    m = np.isfinite(x) & np.isfinite(y)
    fig, ax = plt.subplots(figsize=(8.5,5.5))
    if m.any():
        ax.scatter(x[m], y[m], alpha=0.7, s=24)
    else:
...
def main():
    ...
    for project in projects:
        ...
        if args.cv_mode == "simple":
            ...
            # 検出（reported）基準でテスト期間内に発見された件数を実測Hとする
            vdet = vuln_proj[(vuln_proj["reported_date_date"] >= test_start) & (vuln_proj["reported_date_date"] <= test_end)].copy()
            H_actual_test = int(len(vdet))
    ...
    if rows_fixed:
        ...
        # 図: Effort % of baseline の ECDF とボックス
        out_ecdf_pct = os.path.join(args.out_dir, "fixed_runs_effort_pct_ecdf.png")
        plot_ecdf_single(df_fixed_by_proj["effort_pct_of_baseline"].values,
                         xlabel="Effort (% of Actual)",
                         title=f"ECDF of Effort % (Fixed runs per flag = {int(df_fixed['fixed_runs_per_flag'].iloc[0])})",
                         out_png=out_ecdf_pct,
                         percent_y=True)
        out_box_pct = os.path.join(args.out_dir, "fixed_runs_effort_pct_box.png")
        plot_violin_box_single(df_fixed_by_proj["effort_pct_of_baseline"].values,
                               ylabel="Effort (% of Actual)",
                               title=f"Effort % Distribution (Fixed runs per flag = {int(df_fixed['fixed_runs_per_flag'].iloc[0])})",
                               out_png=out_box_pct,
                               percent_y=True)

        # 図: 期待発見数の ECDF とボックス
        out_ecdf_found = os.path.join(args.out_dir, "fixed_runs_expected_found_ecdf.png")
        plot_ecdf_single(df_fixed_by_proj["expected_found"].values,
                         xlabel="Expected discoveries H(B)",
                         title=f"ECDF of Expected Discoveries (Fixed runs per flag = {int(df_fixed['fixed_runs_per_flag'].iloc[0])})",
                         out_png=out_ecdf_found,
                         percent_y=True)
        out_box_found = os.path.join(args.out_dir, "fixed_runs_expected_found_box.png")
        plot_violin_box_single(df_fixed_by_proj["expected_found"].values,
                               ylabel="Expected discoveries H(B)",
                               title=f"Expected Discoveries Distribution (Fixed runs per flag = {int(df_fixed['fixed_runs_per_flag'].iloc[0])})",
                               out_png=out_box_found,
                               percent_y=False)

        # 図: Effort % vs Expected Found の散布図
        out_scatter = os.path.join(args.out_dir, "fixed_runs_found_vs_effort_pct.png")
        plot_scatter_cost_vs_found(df_fixed_by_proj["effort_pct_of_baseline"].values,
                                   df_fixed_by_proj["expected_found"].values,
                                   x_label="Effort (% of Actual)",
                                   y_label="Expected discoveries H(B)",
                                   title=f"Expected Discoveries vs Effort % (Fixed runs per flag = {int(df_fixed['fixed_runs_per_flag'].iloc[0])})",
                                   out_png=out_scatter,
                                   percent_x=True)
