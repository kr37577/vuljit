...
- `mode` 引数（CLI からは `--strategy1-mode {per_project,cross_project}`）で挙動を切り替え可能。`cross_project` では Fold 情報を参照せず、対象プロジェクト自身の検出サンプルが欠ける場合は「他プロジェクトのみ」の統計（LOPO）から中央値を取得し、必要に応じて従来の全体中央値へフォールバックします。
...
- `mode` を `cross_project` にすると Fold 列が無くても動作し、優先的に「対象プロジェクトを除いた」LOPO IQR からサンプリングします（LOPO が生成できない場合だけ全体 IQR にフォールバック）。CLI では `--strategy2-mode cross_project` で指定します。
...
- `mode="cross_project"`（CLI では `--strategy3-mode cross_project`）を指定すると Fold ベースではなく LOPO の中央値（対象以外の全プロジェクトで計測）を優先し、取得できない場合のみ従来の全体中央値を使用します。これにより全陽性日に対して `line_change_total` の比率で追加ビルドを配分しつつ、予算決定は “他プロジェクトの実績” を基準にできます。`--strategy3-global-budget` で総ビルド量を明示的に指定することも可能で、行変更量の正規化は行わず生値のシェアを利用します。
...
- Simple/Multi モードを CLI で切り替え可能。`--strategy4-mode simple` にすると `line_change_total` 単一特徴量で回帰を行い、この列が欠けている場合は実行時にエラーで早期検知します。デフォルトの `multi` では従来どおり prediction settings 由来の特徴量群を利用します。
- 追加特徴量を使いたい場合は `--strategy4-feature <col>` を複数回指定し、Simple/Multi どちらのモードでも上書き可能です。環境変数 `RQ3_STRATEGY4_MODE` でデフォルトモードを指定できます。
