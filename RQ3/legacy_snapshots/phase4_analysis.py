#!/usr/bin/env python3
"""Phase 4 precision/threshold analysis for RQ3.
...
Phase 3 build strategies under those thresholds to quantify added build load
...
DEFAULT_OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "phase4_outputs")
...
def _iter_prediction_files(root: str) -> Iterable[str]:
    if not os.path.isdir(root):
        return []
    for dirpath, _, filenames in os.walk(root):
        for name in filenames:
            if name.endswith("_daily_aggregated_metrics_with_predictions.csv"):
                yield os.path.join(dirpath, name)


def _load_project_predictions(
    path: str,
    risk_column: str,
    ...
    if not required.issubset(df.columns):
        return None
    df = df[["merge_date", "is_vcc", risk_column]].copy()
    df["project"] = os.path.basename(path).split("_daily_aggregated_metrics_with_predictions.csv")[0]
...
def collect_predictions(
    ...
    for path in _iter_prediction_files(predictions_root):
        df = _load_project_predictions(path, risk_column, label_column)
        if df is not None:
            frames.append(df)
    if not frames:
        raise RuntimeError(f"No prediction files found under {predictions_root!r}.")
